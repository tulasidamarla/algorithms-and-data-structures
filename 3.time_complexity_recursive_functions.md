...
<b>Case 1 examples</b>
 ...
 1) T(n) = 2T(n/2) + 1
	...
	Time complexity : <b>O(n<sup>log<sub>2</sub>2</sup>)	= O(n) </b><i>//as (log<sub>2</sub>2 ) > 0 </i>

 2) T(n) = 4T(n/2) + 1
 
	a = 4, b = 2, k = 0 <i>(1 can be written as n^0)</i><br>
	Time complexity : <b>O(n<sup>log<sub>2</sub>4</sup>)	= O(n<sup>2</sup>)</b> <i>//as (log<sub>2</sub>4 ) > 0 </i>

 3) T(n) = 8T(n/2) + 1
 
	a = 4, b = 2, k = 0 <i>(1 can be written as n^0)</i><br>
	Time complexity : <b>O(n<sup>log<sub>2</sub>8</sup>)	= O(n<sup>3</sup>)</b>  <i>//as (log<sub>2</sub>4 ) > 0 </i>

 4) T(n) = 8T(n/2) + n<sup>2</sup>
	
	a = 8, b = 2, k = 2  
	Time complexity : <b>O(n<sup>log<sub>2</sub>8</sup>)	= O(n<sup>3</sup>)</b>  <i>//as (log<sub>2</sub>8 ) > 2 </i>
   
 5) T(n) = 8T(n/2) + nlog<sub>2</sub>n
	
	a = 8, b = 2, k = 1 
	Time complexity : <b>O(n<sup>log<sub>2</sub>8</sup>)	= O(n<sup>3</sup>)</b> <i>// as (log<sub>2</sub>8  > 1) </i>

 <i><b>Note</b>: As long as log of a base b greater than k, it won't consider the log component of the second term.</i>
 ...
 1) T(n) = 2T(n/2) + n 
	...
	a = 2, b = 2, k = 1, p = 0  <i>//No log component in second term, so p is 0.</i>
	Time complexity is : <b>O(n<sup>k</sup>(logn)<sup>p+1</sup>) = O(nlogn)</b> &nbsp <i>//as (log<sub>2</sub>2 = 1)</i>
