...
<h3>Divide and Conquer</h3>
 ...
 <ins>T(n) = T(n-1) + 1 </ins> // Using asymptotic notation, c0 + c1 can be represented as 1.
...
<b>Time complexity calculation</b>
 ...
 Solution:
		...
		T(n) = T(n-1) + 1
		T(n) = T(n-2) + 2 // by substituting T(n-1) = T(n-2) + 1
		T(n) = T(n-3) + 3 // by substituting T(n-2) = T(n-3) + 1
		....
		T(n) = T(n-k) + k

		T(n) = T(n-n) + n // when k = n

 Hence, time complexity of the problem is <ins>O(n)</ins>.
...
Solution:
		...
		T(n) = 2T(n-1) + 1
		T(n) = 2^2T(n-2) + 2 + 1 // substitute the formula T(n-1) = 2T(n-2) + log(n-1)
		T(n) = 2^3T(n-3) + 2^2 + 2 + 1 // substitute the formula T(n-2) = 2T(n-3) + log(n-2)
		....
...
If T(n) = aT(n-b) + f(n)
- where a > 0, b > 0 and 
- f(n) = O(n<sup>k</sup>) where k > 0, then time complexity 
