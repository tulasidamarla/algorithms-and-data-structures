...
<h3>Divide and Conquer</h3>
 ...
 T(n) = T(n-1) + 1 <i>// Using asymptotic notation, c<sub>0</sub> + c<sub>1</sub> can be represented as 1.</i>
...
<b>Time complexity calculation</b>  
 ...
 T(n) = T(n-1) + log(n) <i>// Using asympotatic notation (c<sub>0</sub> + c<sub>1</sub>)log(n) + c<sub>0</sub> becomes log(n).</i>
 ...
 T(n) = 2T(n-1) + 1 <i>// Assympotatic notation for the above c<sub>1</sub> + c<sub>0</sub> becomes 1.</i>
...
<h5>Deriving masters theorm from the above solved problems</h5>
 ...
 <h4>Masters theorm for decreasing functions</h4>
 ...
 If T(n) = aT(n-b) + f(n) 
	...
	- then time complexity 
		...
		- O(a<sup>n</sup>n<sup>k</sup>) if a > 1
		- O(n<sup>k</sup>) if a < 1
